...
DATE_DIR=$(date +%Y-%-m-%-d)
LOCAL_BACKUP_ARCHIVE="/tmp/local_backup/$DATE_DIR/backup.tar"

if [ ! -f "$LOCAL_BACKUP_ARCHIVE" ]; then
    echo "Local backup archive not found!"
    exit 1
fi

echo "Listing local backup archive contents:"
tar -tf "$LOCAL_BACKUP_ARCHIVE"

mkdir -p /tmp/local_verify
rm -rf /tmp/local_verify/*
tar -xf "$LOCAL_BACKUP_ARCHIVE" -C /tmp/local_verify

if [ ! -f "/tmp/local_verify/backup_test_vol1/file1.txt" ]; then
    echo "File1 not found in local backup!"
    exit 1
fi

if [ -d "/tmp/local_verify/backup_test_vol_excluded" ]; then
...
REMOTE_BACKUP_ARCHIVE="/ssh_config/remote_backup/$DATE_DIR/backup.tar"

if [ ! -f "$REMOTE_BACKUP_ARCHIVE" ]; then
    echo "Remote backup archive not found at $REMOTE_BACKUP_ARCHIVE!"
    exit 1
fi

echo "Listing remote backup archive contents:"
tar -tf "$REMOTE_BACKUP_ARCHIVE"

mkdir -p /tmp/remote_verify
rm -rf /tmp/remote_verify/*
tar -xf "$REMOTE_BACKUP_ARCHIVE" -C /tmp/remote_verify

 if [ -f "/tmp/remote_verify/backup_test_vol1/file1.txt" ]; then
...
if [ -d "/tmp/remote_verify/backup_test_vol_excluded" ]; then
    echo "Excluded volume found in remote backup!"
    exit 1
fi
...
if $BINARY -d testuser@ssh-target:/config/small_remote,unix 2>&1 | grep -q "Not enough space"; then
